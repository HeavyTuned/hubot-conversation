...
var Dialog = function Dialog(originalMessage, timeoutValue, timeoutMessage) {
    ...
    this.receive = function (msg) {
        ...
        if (!matched && choices.length > 0) {
            if(helpmsg != ""){
                msg.reply(helpmsg);
            }else{
                msg.reply("Deine Eingabe war ungültig!");
     ...
     */
    this.addChoice = function (regex, handler, msg) {
        if(msg == undefined){
            msg = "";
        }
        choices.push({regex: regex, handler: handler, helpMessage: msg});
